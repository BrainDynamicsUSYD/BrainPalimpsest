function T = calcTransFuncs_fromPhi_1D(kx, w, params)
%% calcTransFuncs_fromPhi_1D.m
%
% Calculates the 1D transfer functions T.T_Aphi from neural activity phi to A, 
% where A is either the BOLD signal Y, neuroglial drive z, CBF F, CBV Xi, 
% dHb Q, and modes.
%
% Inputs: kx     : vector of 1D spatial frequencies
%         w      : vector of temporal frequencies
%         params : instance of the class loadParameters of the toolbox
%
% Output: T      : structure containing the transfer functions T_Aphi.
...
% >> T.T_Yphi  % gives out the T_Yphi transfer function
%
...
[ww, kkx] = meshgrid(w, kx);

% Transfer function from z to F
T_Fz = 1./(-(ww + 1i*0.5*params.kappa).^2 + params.w_f^2);

% Transfer function from F to Xi
T_XiF = params.rho_f*params.C_z*(params.D/params.rho_f - 1i*ww)./(kkx.^2*params.v_b^2 + params.k_z^2*params.v_b^2 - ww.^2 - 2*params.Gamma*1i*ww + eps);       
...
w1 = -1i*params.Gamma - sqrt(kkx.^2*params.v_b^2 + params.k_z^2*params.v_b^2 - params.Gamma^2);
w2 = -1i*params.Gamma + sqrt(kkx.^2*params.v_b^2 + params.k_z^2*params.v_b^2 - params.Gamma^2);
